...
static void write_global_attributes(GString *outstr, gint indent)
{
	...
	/* Thumbnails Options */
	WRITE_NL(); WRITE_INT(*options, thumbnails.save_width);
...
static gboolean load_global_params(const gchar **attribute_names, const gchar **attribute_values)
{
	while (*attribute_names)
		...
		if (READ_UINT_CLAMP(*options, duplicates_match, 0, DUPE_MATCH_NAME_LOOSE)) continue;
		...
		/* Thumbnails options */
		if (READ_INT_CLAMP(*options, thumbnails.save_width, 16, 512)) continue;
		if (READ_INT_CLAMP(*options, thumbnails.display_width, 16, 512)) continue;
