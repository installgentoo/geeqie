...
static void dupe_thumb_step(DupeWindow *dw)
{
	...
	dw->thumb_item = di;
	thumb_loader_free(dw->thumb_loader);
	dw->thumb_loader = thumb_loader_new(options->thumbnails.save_width, options->thumbnails.display_width);
...
static void dupe_menu_copy_path_cb(GtkWidget *, gpointer data)
{
	...
	file_util_path_list_to_clipboard(dupe_listview_get_selection(dw, dw->listview), TRUE);
}
...
static void dupe_listview_set_height(GtkWidget *listview, gboolean thumb)
{
	...
	g_object_set(G_OBJECT(cell), "height", (thumb) ? options->thumbnails.display_width : -1, NULL);
